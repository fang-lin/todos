...
describe('TodoItem', () => {
    ...
    it('It should has a correct text in default', () => {
        ...
        expect(todoItem.find('.todo-item-text').text()).to.equal('have breakfast');
    });

    it('It should has call props.removeTodo when click remove button', () => {
        const todoItem = mount(<TodoItem {...props}/>);
        todoItem.find('.todo-item-remove').simulate('click');
    ...
    it('It should has a checked input when todo is completed', () => {
        ...
        expect(todoItem.find('.todo-item-checkbox').props().checked).to.be.true;
    });

    it('It should call props.setEditingTodo method when double click text', () => {
        const todoItem = mount(<TodoItem {...props}/>);
        todoItem.find('.todo-item-text').simulate('doubleclick');
    ...
    it('It should has call props.updateTodo', () => {
        const todoItem = mount(<TodoItem {...props}/>);
        const event = {
            target: {
                checked: true
        ...
        todoItem.find('.todo-item-checkbox').simulate('change', event);
